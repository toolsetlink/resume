...
export function ModernTemplate({ data, template }: { data: ResumeData; template: ResumeTemplate }) {
  ...
  return (
    <div className="resume-template resume-template-modern" data-template={template.id} style={templateContainerStyle(template, data)}>
      ...
      <main className="resume-template-main">
        <header className="resume-template-header">{header.map((section) => <div key={section.id}>{section.node}</div>)}</header>
        <div className="resume-template-content" data-pagination-flow="main">{main.map((section) => <div key={section.id} data-pagination-unit>{section.node}</div>)}</div>
      </main>
    </div>
  )
}
