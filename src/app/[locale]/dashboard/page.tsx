      ...
      <div className="p-8 max-w-5xl mx-auto">
        ...
        {resumes.length > 0 ? (
          <div className="border border-[hsl(var(--border-default))] rounded-lg overflow-hidden">
            {resumes.map(resume => (
              <div key={resume.id} className="flex items-center justify-between px-5 py-4 border-b border-[hsl(var(--border-default))] last:border-b-0 hover:bg-[hsl(var(--brand-light))] transition-colors duration-150 cursor-pointer group" onClick={() => router.push(`/workbench/${resume.id}`)}>
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150" onClick={e => e.stopPropagation()}>
                  <Button type="link" size="small" onClick={() => router.push(`/workbench/${resume.id}`)}>{t('common.edit')}</Button>
