  ...
  <div class="border border-[hsl(var(--border-default))] rounded-md mb-2">
    ...
    <div class="flex items-center gap-2 p-3 bg-[hsl(var(--bg-subtle))]">
      ...
      <div class="flex-1 truncate">
        <span class="font-medium">{{ local.title || '未填写标题' }}</span>
        <span v-if="local.subtitle" class="text-[hsl(var(--text-secondary))] ml-2">· {{ local.subtitle }}</span>
      </div>
      ...
      <t-form label-align="top">
        <div class="grid grid-cols-2 gap-3">
          <t-form-item label="标题">
            <t-input v-model="local.title" @blur="commit" />
          </t-form-item>
          <t-form-item label="副标题">
            <t-input v-model="local.subtitle" @blur="commit" />
          </t-form-item>
          <t-form-item label="日期范围">
            <t-input v-model="local.dateRange" placeholder="2020.01 - 2021.01" @blur="commit" />
          </t-form-item>
        </div>
      </t-form>
      <!-- 描述富文本 -->
...
const emit = defineEmits<{ update: [data: Partial<CustomItem>]; remove: [] }>()

const local = reactive<CustomItem>({ ...props.item })
